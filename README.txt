...
**** Added on Jun 11, 2023: Different way to handle References:

Expanding all child nodes when massive number of references are used in spec file resulted in
very large memory footprint and slow perf.

A quick "proof of concept" change is added. It can now handle most cases, except for very extreme
corner cases where it will be unrealistic. Rules should be constructed in such a way to avoid it.

So, change the following:
1. We recorded $ref nodes, then we run a detection of circular ref. 42Crunch actually flagged those as errors. We handle it.
    https://docs.42crunch.com/latest/content/oasv3/oasconformance/structure/v3-validation-reference-circular.htm

   This change is in spec_parse.py. We deprecated addRefChildNodes, instead, we resolveRefs, then addCircularReference to tree
   to report (in one case, there are 161 circular references)
   
2. The case where an API node has
     schema:
       {"$ref": <target>}
   is easily covered, in rules_util.py, under Rule.matchANode, the spec element is taken from the referenced element.
...
3. We also need to check deeper into the reference nodes, in that case, we added a list of "leaf" referenced nodes for checking.
    We maintain in spectree a list of "targetrefnodes", then in s2test_main, after global, perapi checks, we added ref check.
    The rules_util.py, performRefMatches function is almost a copy from the perAPI function


4. There is one very corner case in which, if a rule's identity string straddle between API def and component, like
       labelA->labelB->
               $ref: RefA
       RefA->labelC

    Then the identify labelA->labelB->labelC won't find a match. It is very very rare as I can't imagine an API design
    separating part of a property of an object into another reference.
